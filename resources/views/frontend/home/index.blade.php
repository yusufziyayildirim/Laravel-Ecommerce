            ...
            @if(count($products) > 0)
                <div class="card-group">
                    @foreach($products as $item)
                        <div class="card" >
                            <img style="height: 280px;" src="{{asset("/storage/products/".$item->images[0]->image_url)}}"
                                 class="card-img-top" alt="{{$item->images[0]->alt}}">
                            <div class="card-body">
                                <h5 class="card-title">{{$item->name}}</h5>
                                <h6 class="card-title">Fiyat: {{$item->price}}TL</h6>
                                <p class="card-text">{{$item->lead}}</p>
                                <a href="/sepetim/ekle/{{$item->product_id}}" class="btn btn-primary">Sepete Ekle</a>
                            </div>
                        </div>
                    @endforeach
                </div>
